...
/// Watch-local equivalent of `Views/Watchlist/Sparkline.swift`, kept separate
/// only because it references `WatchTheme` instead of the UIKit-backed
/// `Theme`. Same rendering: recent history (solid) continuing into the best
...
struct WatchSparkline: View {
    ...
    var body: some View {
        Canvas { context, size in
            ...
            context.stroke(projectionPath, with: .color(WatchTheme.accent),
                           style: StrokeStyle(lineWidth: 1.4, lineCap: .round, dash: [3, 2]))
        }
