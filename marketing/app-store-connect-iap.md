...
| Pro Lifetime | Non-Consumable IAP | `com.avaresearch.hummingbird.pro.lifetime` | $49.99 | one-time, no renewal; same features |
...
- Localization (en-US):
  ...
  - Description: `Compare every forecasting method side by side and stretch sketches to 90 days. 7-day free trial. Same public data as Free — never better foresight, never advice.`
...
- Localization (en-US):
  ...
  - Description: `Compare every forecasting method side by side and stretch sketches to 90 days. Same public data as Free — never better foresight, never advice.`

## 2b. Add the Lifetime unlock (In-App Purchases, not the subscription group)
Your app → **Monetization → In-App Purchases** → **Create**:
- Type: **Non-Consumable**
...
- Localization (en-US):
  ...
  - Description: `A one-time unlock of every Pro comparison tool — no renewal, no expiry. Same public data as Free — never better foresight, never advice.`

## 3. Review metadata (each product needs it)
- **Review screenshot:** upload the paywall screenshot (the "Choose your plan" screen showing all three options — `AppStore/raw-screens/08_paywall_plans.png`, re-capture first). The same shot works for every product's slot.
- **Review notes** (paste into each):
  > Pro unlocks on-device comparison convenience only — more forecasting methods to compare side by side and longer (90-day) sketch horizons. It does NOT provide financial advice, buy/sell signals, price targets, or premium data — Free and Pro use the same key-less public data. The Lifetime IAP is the same feature set as the subscription, purchased once. To reach the paywall: Settings (gear) → Hummingbird Pro, or the "Pro" button in the top toolbar.
...
- The 7-day trial is on **Yearly only**; the Lifetime IAP is **Non-Consumable** (no trial, no renewal).
